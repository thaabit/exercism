...
ORDER = %i[straight_flush four fullhouse flush straight three two_pair pair high]
class Poker
  ...
  def hand_value(hand)
    ...
    ranks = hand.map { |card| (MAP[card[0].to_sym] || card[0..-1]).to_i }.sort
  ...
  def best_hand
    max_hand = @hands.max_by { |hand| hand_value(hand) }
    @hands.select { |hand| hand_value(hand) == hand_value(max_hand) }
  end
